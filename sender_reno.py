...
def send_file_tcp_reno():
    """Send file using TCP Reno protocol. Returns (throughput, avg_delay)."""
    # Read file data
            ...
            while True:
                try:
                    ...
                    if ack_id > last_ack:
                        ...
                        # Update cwnd based on slow start or congestion avoidance
                        if cwnd < ssthresh:
                        ...
                        if dup_ack_count == 3 and not in_fast_recovery:
                            ...
                            # Retransmit the packet at window_base
                            ...
                            # Fast recovery: inflate window
                            cwnd += 1
                        
                except BlockingIOError:
                    # No more ACKs in buffer
                    break
            
            udp_socket.setblocking(True)
            udp_socket.settimeout(TIMEOUT)
            
            # If we didn't receive any ACKs, wait with timeout
            if not received_any:
                try:
                    ack_packet, _ = udp_socket.recvfrom(PACKET_SIZE)
                    ack_id, _ = parse_ack(ack_packet)
